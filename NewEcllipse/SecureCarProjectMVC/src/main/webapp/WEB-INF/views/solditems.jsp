	...
	<div class="view jarallax" data-jarallax='{"speed": 0.2}'
							...
							<div class="card-body card-body-cascade">
								<form class="text-center" style="color: #757575;"
									...
									<div class="row">
										...
										<div class="col-lg-4">
											<select class="mdb-select md-form" searchable="Search here.."
												id="modelDropdown" name="model" onchange="createColorDropDown()">
									...
									<div class="row">
										...
										<div class="col-lg-4">
											<select class="browser-default custom-select" 
												id="colorDropdown" name="color" onchange="createTable()">
		...
		<script>
				...
				$
						.ajax({
							...
							success : function(result) {
							   ...
							   // console.log(result)
								var modelInfo = '<option value="" disabled selected>Color</option>';
								for (var i = 0; i < result.length; i++) {
									modelInfo += '<option value =' + result[i].key.color + '>'
											+ result[i].key.color + '</option>';
								}

								$('#colorDropdown').html(modelInfo);
							}
						});
			}
			...
			function createTable() {
				...
				var brandName = $("#brandDropdown option:selected").val();
				var modelName = $("#modelDropdown option:selected").val();
				var color =  $("#colorDropdown option:selected").val();
				$
						.ajax({
							type : 'GET',

							url : "http://localhost:8080/"
									+ "getSoldItems/" + brandName
									+"/"+modelName+"/"+color,
							success : function(result) {
								
							   // console.log(result[0].modelColorKey.model);
								 var tableInfo = '';
								...
								for(var i=0;i<result.length;i++){
									tableInfo += '<tr>' + '<td>'
											+ result[i].modelColorKey.model + ' </td>';
									tableInfo += '<td>' + result[i].modelColorKey.color
											+ '</td>';
									tableInfo += '<td>' + result[i].totalPrice
											+ '</td>';
									tableInfo += '<td>' + result[i].frequency
											+ '</td></tr>';
								}
								tableInfo += '</tbody>'; 
