...
//	}
	...
	protected void configure(HttpSecurity http) throws Exception {
		 ...
		 * "ADMIN") .antMatchers("/user").hasAnyRole("USER",
		 * "ADMIN").antMatchers("/admin").hasAnyRole("ADMIN").anyRequest()
		...
		http.authorizeRequests().antMatchers("/").permitAll()
				// .antMatchers("/","/static/**", "/resources/**","/resources/static/**")
				.antMatchers("/registeruser").permitAll().antMatchers("/register").permitAll().antMatchers("/home")
				.hasAnyRole("USER", "ADMIN").antMatchers("/shop").hasAnyRole("USER", "ADMIN").antMatchers("/category")
				.hasAnyRole("ADMIN").antMatchers("/product").hasAnyRole("ADMIN").anyRequest().authenticated().and()
				.formLogin().loginPage("/login").defaultSuccessUrl("/home").permitAll().and().logout()
				.logoutSuccessUrl("/login?logout=true").permitAll().and().csrf().disable();
	}
