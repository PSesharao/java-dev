...
public class AirlineApp {
	...
	public static void main(String[] args) {
		...
		do {
			...
			case 2:
				...
				try {
					passList = airSer.getPassengersByGenderAndAge(age, gender, flightList);
			...
			case 3:
				...
				try {
					flightL = airSer.getFlightsByDestination(flightList, destination);
				} catch (AirlineServiceException e) {
					e.printStackTrace();
				}
				if (flightL != null)
	...
	private String enterNewFlight(List<Flight> flightList)
	{
		...
		boolean insertToFlightPassenger = false;
		flight.setPassengerList(new ArrayList<>());
		try {
			insertToFlight = airSer.insertIntoFlightDB(flight.getId(), flight.getName(), flight.getSource(),
		...
		if (insertToFlight) {
			...
			for (int i = 0; i < noOfPass; i++) {
				...
				try {
					insertToPassenger = airSer.insertIntoPassengerDB(passenger.getId(), passenger.getName(),
				...
				if (insertToPassenger) {
					...
					try {
						insertToFlightPassenger = airSer.insertIntoFlightPassengerDB(flight.getId(),
								passenger.getId());
					} catch (AirlineServiceException e) {
					...
					if (insertToFlightPassenger)
						System.out.println("Flightpassenger data " + (i + 1) + " got inserted successfully ");
					else
						System.out.println(
								"Flightpassenger data " + (i + 1) + " did not grt inserted successfully ");
	...
	private void displayFlights(List<Flight> flightL) {
		flightL.forEach(flight -> System.out.println(flight.getName()));
	}

	private void displayPassengers(List<Passenger> passList) {
		passList.forEach(pass -> System.out.println(pass.getName()));
