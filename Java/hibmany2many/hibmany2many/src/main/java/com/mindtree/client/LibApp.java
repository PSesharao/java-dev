...
public class LibApp {
	...
	public static void main(String[] args) {
		...
		try {
			...
			libObj.setBooks(new ArrayList<Book>());
			bookObj = getBookDetails();
			bookObj.setLibraries(new ArrayList<Library>());
			bookObj.getLibraries().add(libObj);
			libObj.getBooks().add(bookObj);
			sessionObj.save(bookObj);
			/*
			 * bookObj = getBookDetails(); bookObj.setLibraries(libObj);
			 * libObj.getBooks().add(bookObj); sessionObj.save(bookObj);
			 */
